...
class DialogComponent extends React.Component<DialogProps, DialogState> {
    ...
    renderChildren() {
        ...
        modalProps.ref = (r: HTMLDivElement) => {
            ...
            if (!cacheModalRef) return;
            if (TypeChecker.isFunction(cacheModalRef)) cacheModalRef(r);
            else cacheModalRef.current = r;
        };
		...
		const transitionTimeout = this.props.transitionTimeout || {
			exit: alignment === Alignment.CENTER ? 150 : 300,
			enter: alignment === Alignment.CENTER ? 150 : 300,
		};
